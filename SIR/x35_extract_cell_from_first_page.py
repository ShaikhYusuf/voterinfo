...
try:
    ...
    try:
        ...
        HAS_PYPDF = False
        print("WARNING: pypdf not installed — page height will use hardcoded fallback.")
...
PDF_PAGE_HEIGHT_PT_FALLBACK = 792             # used only when pypdf is unavailable

# ── Cell coordinates in PDF points ───────────────────────────────────────────
# These are fixed for your document layout and never change between files.
# {"page":1, "x":100, "y":417, "width":303, "height":128}
CELL_PT = {"x": 100, "y": 417, "width": 303, "height": 128}

# ── Y-axis origin ─────────────────────────────────────────────────────────────
# "bottom" → PDF spec (y=0 at page bottom)  ← your coordinates use this
...
def get_page_height_pt(pdf_path):
    """
    Read page height in points directly from the PDF mediabox.
    Falls back to PDF_PAGE_HEIGHT_PT_FALLBACK if pypdf is unavailable.
    """
    if not HAS_PYPDF:
        return PDF_PAGE_HEIGHT_PT_FALLBACK
    try:
        reader = PdfReader(pdf_path)
        mb = reader.pages[0].mediabox
        height = float(mb.height)
        dbg(f"page height from PDF: {height:.2f} pt  width: {float(mb.width):.2f} pt")
        return height
    except Exception as e:
        dbg(f"pypdf read failed ({e}), using fallback height {PDF_PAGE_HEIGHT_PT_FALLBACK} pt")
        return PDF_PAGE_HEIGHT_PT_FALLBACK


def cell_crop_box(img_w, img_h, page_height_pt):
    """
    Convert CELL_PT (PDF points) → pixel crop box (left, top, right, bottom).
    page_height_pt is read from the PDF itself, so this works for any page size.
    """
    ...
    if Y_ORIGIN == "bottom":
        top    = int((page_height_pt - y - h) * PT_TO_PX)
        bottom = int((page_height_pt - y)      * PT_TO_PX)
    else:
        top    = int(y       * PT_TO_PX)
        bottom = int((y + h) * PT_TO_PX)
    left, right = max(0, left),  min(right,  img_w)
    top, bottom = max(0, top),   min(bottom, img_h)
    log(f"page_height={page_height_pt:.1f}pt  "
        f"crop (px): left={left} top={top} right={right} bottom={bottom} "
...
def process_pdf(args_tuple):
    ...
    page_height_pt = get_page_height_pt(pdf_path)
    box = cell_crop_box(*page.size, page_height_pt)
